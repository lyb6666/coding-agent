...
二、如何运行
1. 安装：pip install -e .（会同时装好全局命令 coding）
2. 配置密钥：复制 .env.example 为 .env，填入 DEEPSEEK_API_KEY
3. 运行：任意目录敲 coding 进入交互会话（或 python main.py）；coding "任务" 单次执行
